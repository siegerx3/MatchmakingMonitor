	...
	static class RemoteStats
	{
		...
		public static async Task Get(SettingsJson settings)
		{
			...
			await Task.Run(() =>
			{
				...
				settings.AvgXpLimits = Tiers.Select(t => allRegions.Tier(t).AvgXp().OrderedArray().Calc(5 * t, false).LimitsTier(t)).ToArray();

				settings.AvgDmgLimits.Battleship = Tiers.Select(t => allRegions.Tier(t).Type(ShipType.Battleship).AvgDmg().OrderedArray().Calc(500 * t, false).LimitsTier(t)).ToArray();
				settings.AvgDmgLimits.Cruiser = Tiers.Select(t => allRegions.Tier(t).Type(ShipType.Cruiser).AvgDmg().OrderedArray().Calc(500 * t, false).LimitsTier(t)).ToArray();
				settings.AvgDmgLimits.Destroyer = Tiers.Select(t => allRegions.Tier(t).Type(ShipType.Destroyer).AvgDmg().OrderedArray().Calc(500 * t, false).LimitsTier(t)).ToArray();
				settings.AvgDmgLimits.AirCarrier = Tiers.Select(t => allRegions.Tier(t).Type(ShipType.AirCarrier).AvgDmg().OrderedArray().Calc(500 * t, false).LimitsTier(t)).ToArray();
			});
		}
