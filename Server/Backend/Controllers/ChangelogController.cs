...
namespace Backend.Controllers
{
	...
	public class ChangelogController : ApiController
	{
		...
		{
			return await Task.Run(() => Directory.EnumerateFiles(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "changelogs"),
				"*.md",
				SearchOption.TopDirectoryOnly).Select(Path.GetFileNameWithoutExtension).Select(name => new Version(name)).Sort().Select(v => v.ToString()).ToArray());
		}
